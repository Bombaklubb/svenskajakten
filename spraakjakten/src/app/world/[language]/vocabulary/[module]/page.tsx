...
export default function VocabularyModulePage({ params }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/content/${langId}/content.json`)
      ...
      .then((data: LanguageContent) => {
        const mod = data.vocabulary?.find((m) => m.id === moduleId);
  ...
  function finishModule(finalCorrect: number) {
    ...
    const updatedStudent = saveModuleProgress(student, lang!.id, "vocabulary", moduleId, pts, finalCorrect >= Math.ceil(exercises.length * 0.6));
