...
export default function MorfemModulePage({ params }: Props) {
  const { stage: stageId, module: moduleId } = use(params);
  const stage = getStage(stageId);
  const router = useRouter();

  const [student, setStudent] = useState<StudentData | null>(null);
  const [mod, setMod] = useState<MorfemModule | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/content/${stageId}/content.json`)
      ...
      .then((data: StageContent) => {
        const found = data.morfem?.find((m) => m.id === moduleId);
      ...
      <div className="min-h-screen bg-amber-50 dark:bg-gray-900">
        ...
        <main className="max-w-lg mx-auto px-4 py-8 space-y-6">
          ...
          <div className="card text-center space-y-4">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-700">
              🧩 Morfem · Nivå {mod.level}
            </div>
            {mod.helpText && mod.helpText.length > 0 && (
              <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-2xl p-4 text-left space-y-2">
                {mod.helpText.map((tip, i) => (
                  <p key={i} className="text-sm text-purple-800 dark:text-purple-200">💡 {tip}</p>
                ))}
              </div>
            )}
          ...
          <button
            ...
            style={{ background: "linear-gradient(135deg, #a855f7, #7c3aed)", boxShadow: "0 4px 0 0 #6d28d9" }}
          >
            Börja övningen →
          </button>
        </main>
