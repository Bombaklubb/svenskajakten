  ...
  return (
    <div className={`min-h-screen ${getThemeClassName(student.equippedTheme)} ${getThemeWrapperClass(student.equippedTheme)}`}>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all duration-200 cursor-pointer ${
                  tab === t.id
                    ...
                    : "text-sv-800 dark:text-gray-300 hover:bg-sv-50 dark:hover:bg-gray-700"
                }`}
              >
                {t.label}
              </button>
        ...
        {tab === "ramar" && (
          <BlurFade>
            <h2 className="on-theme-muted text-xs font-black uppercase tracking-wider text-sv-800 dark:text-gray-300 mb-3">Ramar runt din avatar</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {FRAMES.map((f) => {
              ...
              {(() => {
                ...
                return (
                  <div className="card flex flex-col items-center text-center !p-4">
                    ...
                    <div className="my-1.5"><span className="text-[10px] font-black uppercase tracking-wider text-sv-800 dark:text-gray-300">Original</span></div>
                    <div className="w-full mt-2">
                      {isStandard ? (
                        <button disabled className="w-full py-2 rounded-xl font-bold text-sm bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border-2 border-emerald-300 dark:border-emerald-700 cursor-default">
                          ✓ Vald
        ...
        {tab === "effekter" && (
          <BlurFade>
            <h2 className="on-theme-muted text-xs font-black uppercase tracking-wider text-sv-800 dark:text-gray-300 mb-3">Effekter runt din avatar</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {EFFECTS.map((e) => {
                ...
                return (
                  <div key={e.id} className="card flex flex-col items-center text-center !p-4">
                    <div className="relative w-full h-24 rounded-xl mb-2 overflow-hidden" style={{ background: "linear-gradient(160deg, #1e293b, #0f172a)" }}>
                      <div className="absolute inset-0 flex items-center justify-center"><FramedAvatar avatar={currentAvatar} frameId={student.equippedFrame} effectId={e.id} size={52} /></div>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {ownedFrames.length === 0 ? (
                  <p className="text-sm text-sv-800 dark:text-gray-300 bg-white dark:bg-gray-800 rounded-2xl border-2 border-dashed border-sv-200 dark:border-gray-600 px-4 py-6 text-center">
